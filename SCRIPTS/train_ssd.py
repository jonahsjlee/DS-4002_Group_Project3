...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent.parent / "OUTPUT" / "checkpoints_ssd",
        help="Checkpoints (ssd_epoch*.pt, ssd_last.pt).",
    )
    parser.add_argument(
        "--metrics-json",
        type=Path,
        default=Path(__file__).resolve().parent.parent / "OUTPUT" / "metrics_ssd_test.json",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Checkpoint from train_ssd (.pt). Used with --eval-only or to init weights.",
    )
    parser.add_argument(
        "--max-train-batches",
        type=int,
    ...
    if args.eval_only and resume_path is None:
        resume_path = args.output_dir / "ssd_last.pt"
    ...
    if not args.eval_only:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            ckpt = args.output_dir / f"ssd_epoch{epoch:02d}.pt"
            torch.save(
                {
                    "epoch": epoch,
        ...
        last_path = args.output_dir / "ssd_last.pt"
        torch.save(
            {
                "epoch": args.epochs,
                "model_state_dict": model.state_dict(),
